...
void ofApp::update() {
	...
	for (int x_param = -1; x_param <= 1; x_param += 2) {
		...
		for (float x = 50; x < 150; x += x_span) {
			...
			float angle_x = ofMap(ofNoise(noise_seed.x, (ofGetFrameNum() + x) * 0.005), 0, 1, -360, 360);
			auto rotation_x = glm::rotate(glm::mat4(), angle_x * (float)DEG_TO_RAD, glm::vec3(1, 0, 0));

			float angle_y = ofMap(ofNoise(noise_seed.y, (ofGetFrameNum() + x) * 0.005), 0, 1, -360, 360);
			auto rotation_y = glm::rotate(glm::mat4(), angle_y * (float)DEG_TO_RAD, glm::vec3(0, 1, 0));

			float next_angle_x = ofMap(ofNoise(noise_seed.x, (ofGetFrameNum() + x + 1) * 0.005), 0, 1, -360, 360);
			auto next_rotation_x = glm::rotate(glm::mat4(), next_angle_x * (float)DEG_TO_RAD, glm::vec3(1, 0, 0));

			float next_angle_y = ofMap(ofNoise(noise_seed.y, (ofGetFrameNum() + x + 1) * 0.005), 0, 1, -360, 360);
			auto next_rotation_y = glm::rotate(glm::mat4(), next_angle_y * (float)DEG_TO_RAD, glm::vec3(0, 1, 0));

			vector<glm::vec3> vertices;
			vertices.push_back(glm::vec4(x * x_param, 5, 0, 0) * rotation_y * rotation_x);
			vertices.push_back(glm::vec4(next_x * x_param, 5, 0, 0) * next_rotation_y * next_rotation_x);
			vertices.push_back(glm::vec4(next_x * x_param, -5, 0, 0) * next_rotation_y * next_rotation_x);
			vertices.push_back(glm::vec4(x * x_param, -5, 0, 0) * rotation_y * rotation_x);
